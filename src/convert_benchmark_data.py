...
def fetch_papers_data(paper_ids: list, output_dir: str, batch_size: int = 10):
    """
    Fetch paper data from Semantic Scholar API and save in output_dir/data/ss_papers.json
    """
    ...
    def process_paper_response(response):
        ...
        return {
            ...
            "references": [ref["paperId"] for ref in response.get("references", []) if ref.get("paperId")]
        }
    ...
    for idx in range(0, len(remaining_ids), batch_size):
        ...
        for retry in range(max_retries):
            try:
                # Update API request to include references
                response = requests.post(
                    "https://api.semanticscholar.org/graph/v1/paper/batch",
                    headers={'x-api-key': api_key} if api_key else {},
                    params={'fields': 'paperId,abstract,publicationDate,authors,fieldsOfStudy,title,referenceCount,year,citationCount,s2FieldsOfStudy,references.paperId'},
                    json={"ids": batch_ids},
                    timeout=30
                )
...
def get_openreview_ids(papers_dir: Path):
    ss_paper_ids = [f.stem for f in papers_dir.glob("*.json")]
    # remove "no_ss" from ss_paper_ids - there should be 1
    ss_paper_ids = [paper_id for paper_id in ss_paper_ids if paper_id != "no_ss"]

    print(f"Found {len(ss_paper_ids)} papers")
    print(f"Paper 1: {ss_paper_ids[0]}")
    assert len(ss_paper_ids) == 3373, "Expected 3374 papers"

    return ss_paper_ids

def get_mag_ids(papers_dir: Path):
    ss_paper_ids = [f.stem for f in papers_dir.glob("*.json")]
    print(f"Found {len(ss_paper_ids)} papers")
    print(f"Paper 1: {ss_paper_ids[0]}")
    assert len(ss_paper_ids) == 1000, "Expected 1000 papers"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--type", type=str, choices=["openreview", "mag"], default="openreview", help="Which benchmark data to convert")
    args = parser.parse_args()

    test_output_dir(args.output_dir)

    if args.type == "openreview":
        # TODO: abstract this
        ...
        paper_ids = get_openreview_ids(OR_PAPERS_DIR)

    elif args.type == "mag":
        # TODO: abstract this
        MAG_PAPERS_DIR = Path("/Users/thomaswiener/Arxiv-Recommender/ArXiv_Recommendation_Research_v2/Data/mag_papers")
        if not MAG_PAPERS_DIR.exists():
            raise FileNotFoundError(f"Directory {MAG_PAPERS_DIR} does not exist.")

        paper_ids = get_mag_ids(MAG_PAPERS_DIR)

    # Fetch and save paper data
    fetch_papers_data(paper_ids, args.output_dir, args.batch_size)
